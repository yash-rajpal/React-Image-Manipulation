...
const CanvasCrop = ({ img: imageAdd, setImg, reset }) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      if (coords) {
        ...
        contextRef.current.rect(x1 - divPos.x, y1 - divPos.y, x2 - x1, y2 - y1);
  ...
  const _onMouseDown = (e) => {
    ...
    setCoords({ x1: clientX, y1: clientY });
  };
  const _onMouseMove = (e) => {
    if (!isDrawing) return;
    const { x1, y1 } = coords;
    const { clientX, clientY } = e;
    setCoords({ x1, y1, x2: clientX, y2: clientY });
  };
  const _onMouseUp = (e) => {
    setIsDrawing(false);
  };
